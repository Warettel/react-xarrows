...
var getPosition = function (xProps, mainRef) {
    ...
    if (path === 'grid') {
        if (roundedCorners && !(cpx1 === cpx2 && cpy1 === cpy2)) {
            if (x2 === cpx2 && y2 === cpy2) {
                // 2 lines, 1 corner
                var roundedEdge = getRoundedEdge({ x: x1, y: y1 }, { x: cpx1, y: cpy1 }, { x: x2, y: y2 });
                arrowPath = "M " + x1 + " " + y1 + " " + roundedEdge + " L " + x2 + " " + y2;
            }
            else {
                // 3 lines, 2 corners
                var roundedEdge1 = getRoundedEdge({ x: x1, y: y1 }, { x: cpx1, y: cpy1 }, { x: cpx2, y: cpy2 });
                var roundedEdge2 = getRoundedEdge({ x: cpx1, y: cpy1 }, { x: cpx2, y: cpy2 }, { x: x2, y: y2 });
...
var getRoundedEdge = function (start, corner, end) {
    var cornerStartXOffset = 0;
    var cornerStartYOffset = 0;
    var cornerEndXOffset = 0;
    var cornerEndYOffset = 0;
    if (corner.x === start.x) {
    ...
    return ("L " + (corner.x + cornerStartXOffset) + " " + (corner.y + cornerStartYOffset) + " " +
